...
namespace Task6
{
    public partial class Form1:Form
    {
        ...
        public Form1()
        {
            ...
            dataGridView1.Columns.Add("c1",$"{dataGridView1.ColumnCount + 1}");
        ...
        private void button1_Click(object sender,EventArgs e)
        {
            ...
            int[,] result;
            //Объявление переменной для хранения индексов оставшихся строк
            int[] indexes;
            // Массив максимальных элементов
            int[] max = Class2DArray.FindMax(array,out result, out indexes);
            // Сброс размера формы
            Size = startSize;
            ...
            if (result != null)
            {
                ...
                {
                    dataGridView2.Rows[i].HeaderCell.Value = (indexes[i]+1).ToString();
                }
        ...
        private void numericUpDown2_ValueChanged(object sender,EventArgs e)
        {
            ...
            {
                dataGridView1.Columns.Add($"c{dataGridView1.ColumnCount}", $"{dataGridView1.ColumnCount+1}");
                dataGridView1.Columns[dataGridView1.ColumnCount - 1].SortMode = DataGridViewColumnSortMode.NotSortable;
                dataGridView1.Columns[dataGridView1.ColumnCount - 1].Width = columnwidth;
            }
            // Удаление столбцов
        ...
        void ResizeDGV(DataGridView dgv)
        {
            ...
            {
                dgv.Size = new Size(columnwidth * 7 + 20 + dgv.RowHeadersWidth, dgv.RowTemplate.Height * 7 + 20 + dgv.ColumnHeadersHeight);
            }
            else if(dgv.RowCount > 7 && dgv.ColumnCount <= 7)
            {
                dgv.Size = new Size(columnwidth * dgv.ColumnCount + 20 + dgv.RowHeadersWidth, dgv.RowTemplate.Height * 7 + 3 + dgv.ColumnHeadersHeight);
            }
            else if(dgv.RowCount <= 7 && dgv.ColumnCount > 7)
            {
                dgv.Size = new Size(columnwidth * 7 + 3 + dgv.RowHeadersWidth, dgv.RowTemplate.Height * dgv.RowCount + 20 +3 + dgv.ColumnHeadersHeight);
            }
            else
            {
                dgv.Size = new Size(columnwidth * dgv.ColumnCount + 3 + dgv.RowHeadersWidth, dgv.RowTemplate.Height * dgv.RowCount +3+ dgv.ColumnHeadersHeight);
            }
        }
